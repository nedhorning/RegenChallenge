...
setwd("/home/nedhorning/ned/NewJobStuff/RegenChallenge/RegenChallenge/QGIS_Files")
...
classSampNums <- c(5000,5000,5000,5000)
...
inImageName <- "Tanzania1993_2016_clipped.tif"
...
if(multiprocessing){
  ...
  if (classImage || threshImage) {
    ...
    class_calc <- calc(pred_calc, fun=classPred, filename=outClassImage, overwrite=TRUE)
  ...
  if (probImage || threshImage){
    ...
    prob_calc <- calc(pred_calc, fun=probPred, filename=outProbImage, overwrite=TRUE)
    # Multiprocessing Implementation:
    # Works, but is often slower
  ...
  if(threshImage){
    ...
    thresh_calc <- overlay(class_calc, prob_calc, fun=threshPred, filename=outThreshImage, overwrite=TRUE)
  }
